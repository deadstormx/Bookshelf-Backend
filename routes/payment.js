  ...
  try {
    ...
    if (data.status === "Completed") {
      ...
      await Notification.create({
        ...
        message: `Your order #${order._id.toString().slice(-6).toUpperCase()} has been confirmed! Total: Rs ${order.totalAmount.toFixed(2)}`,
        type:    "order",
      });
  ...
  try {
    ...
    if (status === "delivered") {
      await Notification.create({
        ...
        message: `Your order #${order._id.toString().slice(-6).toUpperCase()} has been delivered! Enjoy your books.`,
        type:    "order",
      });
    }
    res.json(order);
...
router.post("/initiate-cod", protect, async (req, res) => {
  ...
  try {
    ...
    await Notification.create({
      user:    req.user._id,
      title:   "Order Placed 📦",
      message: `Your COD order #${order._id.toString().slice(-6).toUpperCase()} has been placed! Total: Rs ${totalAmount.toFixed(2)}. Pay on delivery.`,
      type:    "order",
    });
